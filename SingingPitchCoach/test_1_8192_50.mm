...
-(id)initWithSize:(CGSize)size
{
    if (self = [super initWithSize:size])
    {
        ...
        /* start the mic */
        _audioController = [[AudioController alloc] init:44100 FrameSize:8192];
        _bufferManager = [_audioController getBufferManagerInstance];
        _l_fftData = (Float32*) calloc(8192, sizeof(Float32));
        _l_cepstrumData = (Float32*) calloc(8192, sizeof(Float32));
        _l_fftcepstrumData = (Float32*) calloc(8192, sizeof(Float32));
        _Hz120 = floor(120*(float)8192/(float)44100);
        _Hz530 = floor(530*(float)8192/(float)44100);
